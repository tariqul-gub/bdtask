...
@section('content')
<div class="mb-4">
    <h1 class="h2">Edit Journal Entry #{{ $journalEntry->id }}</h1>
...
<form action="{{ route('journal-entries.update', $journalEntry) }}" method="POST" id="journalEntryForm">
                    ...
                    <select class="form-select @error('branch_id') is-invalid @enderror" 
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" 
                                {{ old('branch_id', $journalEntry->branch_id) == $branch->id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
    ...
    <div class="d-flex gap-2">
        <button type="submit" class="btn btn-primary">
            <i class="bi bi-save"></i> Update Journal Entry
        </button>
...
const existingLines = @json($journalEntry->journalLines);

function addLine(accountId = '', debit = 0, credit = 0) {
...
existingLines.forEach(line => {
    addLine(line.account_id, line.debit, line.credit);
});
</script>
@endsection
