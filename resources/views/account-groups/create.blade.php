...
@section('content')
<div class="mb-4">
    <h1 class="h2">Create New Account Group</h1>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('parent_id') is-invalid @enderror" 
                            ...
                            @foreach($parentGroups as $parentGroup)
                                <option value="{{ $parentGroup->id }}" {{ old('parent_id') == $parentGroup->id ? 'selected' : '' }}>
                                    {{ $parentGroup->name }}
                                </option>
                            @endforeach
                        </select>
                        @error('parent_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Select a parent group to create a sub-group</small>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-save"></i> Save Account Group
                        </button>
